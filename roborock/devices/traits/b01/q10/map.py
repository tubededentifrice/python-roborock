...
class MapDps(RoborockBase):
    ...
    restricted_zone_up: bytes | str | None = field(default=None, metadata={"dps": B01_Q10_DP.RESTRICTED_ZONE_UP})
    virtual_wall_up: bytes | str | None = field(default=None, metadata={"dps": B01_Q10_DP.VIRTUAL_WALL_UP})
...
class MapContentTrait(TraitUpdateListener):
    ...
    def map_data(self) -> MapData | None:
        """Parsed map metadata and any placed vector data."""
    ...
    def layers(self) -> GridLayers | None:
        """Separable background, wall, floor and room layers."""
    ...
    def update_from_map_packet(self, packet: Q10MapPacket) -> None:
        ...
        render = self._compose(source, self.calibration)
        if render is None:
            return
        self._source = source
        self._render = render
    ...
    def update_from_trace_packet(self, packet: Q10TracePacket) -> None:
        ...
        if self.calibration is not None:
            self._rebuild(self.calibration)
        self._notify_update()

    def solve_calibration(self) -> GridCalibration | None:
        """Solve calibration from the grouped map/trace source and rebuild."""
        map_packet = self._source.map_packet
        if self._render is None or map_packet is None:
            return None
        calibration = solve_q10_calibration(self._render.layers, map_packet.header_calibration, self.path)
        if calibration is not None:
            self._rebuild(calibration)
        return calibration
    ...
    def _compose(self, source: Q10MapSource, calibration: GridCalibration | None) -> Q10MapRender | None:
        ...
        try:
            return render_q10_map(
                ...
                path=source.trace_packet.points if source.trace_packet else [],
                robot_position=source.trace_packet.robot_position if source.trace_packet else None,
                zones=self.zones,
                virtual_walls=self.virtual_walls,
                config=self._config,
            )
    ...
    def _rebuild(self, calibration: GridCalibration | None) -> None:
        """Replace the derived render from the current source snapshot."""
        render = self._compose(self._source, calibration)
